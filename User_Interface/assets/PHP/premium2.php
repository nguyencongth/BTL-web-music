...
<body>
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (empty($_POST["ACCOUNTNUMBER"])) {
            $erorr["ACCOUNTNUMBER"] = "vui lòng nhập ACCOUNT NUMBER";
        } else {
    ...
    if (isset($_POST["btn"])) {
        if (empty($erorr["ACCOUNTNUMBER"])) {
            if ($BANKNAME == "BIDV" && !preg_match("/^(125|213|581|601|452)+([0-9]{6,11})$/", $ACCOUNTNUMBER)) {
        ...
        if (empty($erorr)) {
            mysqli_query($conn, "INSERT into pay value('','$BANKNAME','$ACCOUNTNUMBER','$NAME','$ID')");
            header("location: premium3.php");
        }
    }
    ?>
    <div id="main">
                    ...
                    <form method="POST">
                        ...
                        <?php if (isset($erorr["BANKNAME"])) { ?>
                            <span id="thongbao"><?php echo $erorr["BANKNAME"];
                                                echo '<br>'; ?></span>
                        <?php } ?>
                        <label for="txtACCOUNTNUMBER">Số tài khoản <br> <input class="input" id="txtACCOUNTNUMBER" name="ACCOUNTNUMBER" type="text" value="<?php if (isset($ACCOUNTNUMBER)) { echo $ACCOUNTNUMBER;} ?>"></label> <br>
                        ...
                        <?php if (isset($erorr["eACCOUNTNUMBER"])) { ?>
                            <span id="thongbao"><?php echo $erorr["eACCOUNTNUMBER"];
                                                echo '<br>'; ?></span>
                        <?php } ?>
                        <label for="txtNAME">Tên người dùng <br><input class="input" id="txtNAME" name="NAME" type="text" value="<?php if (isset($NAME)) { echo $NAME;} ?>"></label> <br>
                        ...
                        <?php if (isset($erorr["eNAME"])) { ?>
                            <span id="thongbao"><?php echo $erorr["eNAME"];
                                                echo '<br>'; ?></span>
                        <?php } ?>
                        <input class="input btn1" type="submit" name="btn" value="xong">
                    </form>
                </div>
                <div id="item"><img src="../img/pre.png" alt=""></div>
            </div>
        </div>
        <div id="footer">
